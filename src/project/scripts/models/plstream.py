...
class unsupervised_OSA(MapFunction):
    ...
    def eval(self, tweets, model):
        for tweet in tweets:
            pred = self.predict(tweet, model)
            self.predictions.append(pred)
            self.labelled_dataset += str(pred)+"\n"
...
def plstream(python_path, data_path, train=True):
    ...
    # checlist stream: [560.001, 647.470] (87.470)
    training_stream = data_stream[:10**5]
    ...
    print('-- Load most recent model and predict checklist')
    params = redis.StrictRedis(host='localhost', port=6379, db=0)
    model =  pickle.loads(params.get('osamodel'))
    predictor = unsupervised_OSA()
    preds = predictor.eval([word_tokenize(i[0]) for i in checklist_stream], model)
    print('> Done\n')

    print('-- Writing Results into predictions/plstream')

    # convert predictions into dataframe
    df = pd.DataFrame(preds)
    print(np.unique(df[0], return_counts=True))
    df[0] = df[0].map({1: 2}).fillna(df[0]) # map 1 to 2
    df[3] = np.zeros(df.shape[0]) # add col of 0's for neutral
    df[0] = df[0].astype(int)
    df = df[[0, 1, 3, 2]] # reorder cols to match checklist format

    df.to_csv('predictions/plstream', sep=' ', index=False)
    print('> Done\n')
